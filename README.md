...
> 2. 一般的VPS厂商防止邮件轰炸基本上都是关了邮件端口的，qq官方的机器人也是关闭使用了的，所以得使用[Qmsg推送机器人](https://qmsg.zendee.cn/)
> 3. 注意内容不要违规，或者你也可以自己搭一个私有云机器人

## install-docker.sh
install-docker.sh脚本是从[docker官方脚本](https://get.docker.com)下载的，其中源硬编码为Aliyun。也可以使用命令：
```shell
bash <(curl -fL https://get.docker.com) docker --mirror Aliyun
```